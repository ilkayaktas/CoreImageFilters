...
class CMYKLevels: CIFilter {
    ...
    override var outputImage: CIImage! {
        guard let inputImage = inputImage,
            let kernel = kernel else { return nil }
...
class CMYKRegistrationMismatch: CIFilter
{
    ...
    override var outputImage: CIImage? {
        ...
        let final = kernel.apply(extent: inputImage.extent, roiCallback: { (index, rect) in
                return rect.insetBy(dx: -1, dy: -1) }, arguments: [inputImage, inputCyanOffset, inputMagentaOffset, inputYellowOffset, inputBlackOffset] )
